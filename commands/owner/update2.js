  ...
  run: async (client, m) => {
    exec('git pull', async (error, stdout, stderr) => {
      ...
      if (stdout.includes('Already up to date.')) {
        ...
        const cambios = stdout.match(/(\d+) files? changed, (\d+) insertions?\(\+\), (\d+) deletions?\(-\)/)
        if (cambios) {
          const [, files, insertions, deletions] = cambios
          msg = `*Actualización completada*\n\nArchivos modificados > ${files}\n+${insertions} lines (se añadieron ${insertions} lineas)\n-${deletions} (se eliminaron ${deletions} lineas)`
        } else {
          msg = `*Actualización completada*\n\n${stdout}`
        }
      }
