...
// Intentar parsear el JSON de interactiveResponseMessage
...
if (m.message?.buttonsResponseMessage) {
  ...
  const cmd = global.comandos.get(id.toLowerCase())
  if (cmd) {
    return await cmd.run(client, m, [], '', id.toLowerCase(), '')
  }
}
...
if (m.message?.listResponseMessage) {
  ...
  // convertir en mensaje normal (CLAVE)
  m.message.conversation = id
  m.text = id
...
// INTERACTIVE BUTTONS (single_select / quick_reply)
if (m.message?.interactiveResponseMessage?.nativeFlowResponseMessage?.paramsJson) {
  try {
    const parsed = JSON.parse(m.message.interactiveResponseMessage.nativeFlowResponseMessage.paramsJson)
    const id = parsed.id || ''
...
// SOLO LOGUEAR SI EMPIEZA CON ™
if (body.startsWith('™') || m.text?.startsWith('™')) {
...
${chalk.bold.white(`${v} Mensaje: ${gradient('orange', 'red')(body || m.text)}`)}
${t}`)
  }
}
